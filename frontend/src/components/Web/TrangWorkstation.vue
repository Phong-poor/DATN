...
const handleAddToCart = async (product) => {
  ...
  if (!token) {
    swal.confirm('Yêu cầu đăng nhập', 'Vui lòng đăng nhập trước khi thêm sản phẩm vào giỏ hàng!', 'Đăng nhập')
      .then((isConfirmed) => {
        if (isConfirmed) {
          router.push({ path: '/dang-nhap', query: { redirect: '/workstation' } })
        }
      })
    return
  }
      ...
      <div class="grid-container">
        ...
        <div class="category-cards-grid scroll-reveal reveal-stagger">
          <div
            ...
            :key="cat.id"
            @click="router.push({ path: '/san-pham', query: { category: 'Workstation', q: cat.query } })"
    ...
    <section class="section magazine-news-section">
      <div class="grid-container">
        ...
        <div class="magazine-grid scroll-reveal reveal-stagger">
          <article v-for="art in articles" :key="art.id" @click="router.push('/tin-tuc')" class="magazine-card">
