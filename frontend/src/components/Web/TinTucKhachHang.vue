...
const fetchPopular = async () => {
  try {
    ...
    const payload = paginationPayload(data)
    popularPosts.value = mapNewsPosts(payload.items)
      .sort((a, b) => Number(b.views || 0) - Number(a.views || 0))
      .slice(0, 3)
    popularPosts.value.forEach(cachePostDetail)
  } catch (error) {
      ...
      <div class="news-main">
        ...
        <template v-else>
          ...
          <div v-if="randomPost" class="editorial-hero-showcase">
            <RouterLink :to="'/tin-tuc/' + randomPost.id" class="hero-link-wrapper">
          ...
          <div class="magazine-grid">
            <RouterLink v-for="post in posts" :key="post.id" :to="'/tin-tuc/' + post.id" class="magazine-card">
