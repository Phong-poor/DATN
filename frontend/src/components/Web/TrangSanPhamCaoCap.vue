...
const clearAllFilters = () => {
  ...
  window.history.replaceState(window.history.state, '', isMacbookRoute.value ? '/macbook' : '/san-pham')
}
...
const goToSection = async (sectionId) => {
  await router.push({ path: '/san-pham', query: route.query, hash: `#${sectionId}` })
  window.setTimeout(() => {
...
const goToPremiumProduct = (product) => {
  ...
  router.push({ path: '/san-pham', query: { q: product?.name || '' } })
}
    ...
    <section id="showroom-section" class="showroom-immersive-banner">
      ...
      <div class="showroom-immersive-container">
        ...
        <div class="showroom-immersive-copy">
          ...
          <button class="btn-glow" @click="router.push('/lien-he')">Đăng ký trải nghiệm ngay</button>
        </div>
