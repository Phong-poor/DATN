import type { VueInspectorClient } from '../index'
...
declare global {
  interface Window {
    ...
    [OPEN_HANDLER_KEY]?: (file: string) => unknown
    [DISPOSE_KEY]?: () => void
  }
}
...
function applyStyles(el: HTMLElement | SVGElement, styles: Record<string, string>): void {
...
function setLogoColors(svg: SVGSVGElement, colors: string[]): void {
...
function createRect(fill: string, stroke: string): SVGRectElement {
...
function isKeyActive(key: string, event: KeyboardEvent): boolean {
  switch (key) {
    case 'shift':
    case 'control':
    case 'alt':
...
function onKeydown(event: KeyboardEvent): void {
  const combo =
    typeof inspectorOptions.toggleComboKey === 'string'
      ? inspectorOptions.toggleComboKey.toLowerCase().split('-')
      : false
  if (!combo || event.repeat || event.key === undefined) return

  if (combo.every((key: string) => isKeyActive(key, event))) toggleEnabled()
}
...
function openInEditor(
  ...
  const target =
    baseUrl instanceof URL ? baseUrl.searchParams.get('file') : `${file}:${line}:${column}`
  const targetFile = target ?? ''

  const devtoolsOpenInEditor = window[OPEN_HANDLER_KEY]
  const promise =
    devtoolsOpenInEditor && targetFile
      ? Promise.resolve(devtoolsOpenInEditor(targetFile))
      : fetch(
          baseUrl instanceof URL
            ? baseUrl
            : `${baseUrl}/__open-in-editor?file=${encodeURIComponent(targetFile)}`,
          { mode: 'no-cors' },
        )

  if (inspectorOptions.disableInspectorOnEditorOpen) void promise.then(disable)

  return promise
}

function handleClick(info: ElementTraceInfo): void {
  closeOverlay()
  const url = new URL(
    `${inspectorOptions.base}__open-in-editor?file=${encodeURIComponent(info.fullpath)}`,
    import.meta.url,
  )
  void openInEditor(url)
}

const client: VueInspectorClient = {
  get enabled() {
  ...
  disable,
  toggleEnabled,
  openInEditor: openInEditor as VueInspectorClient['openInEditor'],
  onUpdated() {},
  onEnabled() {},
  onDisabled() {},
}
...
function setRect(rect: SVGRectElement, domRect: DOMRect): void {
...
function setupOverlay(): () => void {
  ...
  banner.innerHTML =
    '<div>vite-plugin-vue-inspector</div><div class="tip">Click on a element › Open IDE › Link to File</div>'
  ...
  toggle.addEventListener('click', (event) => {
    if ((event.target as Element | null)?.closest?.('.vue-inspector-banner')) return
  ...
  const stopToggleEffect = watchEffect(() => {
    ...
    const colors = enabled ? ['#42D392', '#213547', '#42b883'] : ['#E2C6C6', '#E2C6C6', '#E2C6C6']

    setLogoColors(logo, colors)
    toggle.classList.toggle('is-disabled', !enabled)

    const { toggleButtonVisibility } = inspectorOptions
    toggle.style.display =
      toggleButtonVisibility === 'always' || (toggleButtonVisibility === 'active' && enabled)
        ? 'block'
        : 'none'
