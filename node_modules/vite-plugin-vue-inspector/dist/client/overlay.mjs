...
.vue-inspector-banner .tip,
.vue-inspector-floats .tip {
  opacity: 0.7;
  font-size: 11px;
}
.vue-inspector-toggle:hover .vue-inspector-banner {
  display: block;
}
.vue-inspector-toggle.is-disabled:hover .vue-inspector-banner {
  display: none;
}
.vue-inspector-svg {
  position: fixed;
...
.vue-inspector-animated {
  transition: all ${ANIMATE_DURATION} ease-in;
}
...
function createLogoSvg() {
	...
	template.innerHTML = logoSvgContent.trim();
...
function setLogoColors(svg, colors) {
	const paths = svg.querySelectorAll("path");
	const stops = svg.querySelectorAll("stop");
	paths[0]?.setAttribute("fill", colors[0]);
	paths[2]?.setAttribute("fill", colors[1]);
	paths[3]?.setAttribute("fill", colors[2]);
	stops[0]?.setAttribute("stop-color", colors[0]);
	stops[1]?.setAttribute("stop-color", colors[2]);
}
function createRect(fill, stroke) {
...
function update(result) {
	...
	const [file, line, column] = result.pos;
	linkParams.file = file;
	linkParams.line = line;
	linkParams.column = column;
	linkParams.title = file;
...
function refreshOverlay() {
	if (state.isVisible) update(lastMatchedElement.value);
}
...
function enable() {
	if (!isEnabled.value) toggleEnabled();
}
function disable() {
	if (isEnabled.value) toggleEnabled();
}
function isKeyActive(key, event) {
	switch (key) {
		...
		case "control":
		case "alt":
		case "meta": return event.getModifierState(key.charAt(0).toUpperCase() + key.slice(1));
		default: return key === event.key.toLowerCase();
	}
}
function onKeydown(event) {
	const combo = typeof inspectorOptions.toggleComboKey === "string" ? inspectorOptions.toggleComboKey.toLowerCase().split("-") : false;
	if (!combo || event.repeat || event.key === void 0) return;
	if (combo.every((key) => isKeyActive(key, event))) toggleEnabled();
}
function openInEditor(baseUrl, file, line, column) {
	const targetFile = (baseUrl instanceof URL ? baseUrl.searchParams.get("file") : `${file}:${line}:${column}`) ?? "";
	const devtoolsOpenInEditor = window[OPEN_HANDLER_KEY];
	const promise = devtoolsOpenInEditor && targetFile ? Promise.resolve(devtoolsOpenInEditor(targetFile)) : fetch(baseUrl instanceof URL ? baseUrl : `${baseUrl}/__open-in-editor?file=${encodeURIComponent(targetFile)}`, { mode: "no-cors" });
	if (inspectorOptions.disableInspectorOnEditorOpen) promise.then(disable);
	return promise;
}
function handleClick(info) {
	closeOverlay();
	openInEditor(new URL(`${inspectorOptions.base}__open-in-editor?file=${encodeURIComponent(info.fullpath)}`, import.meta.url));
}
const client = {
	get enabled() {
...
function setRect(rect, domRect) {
	rect.setAttribute("x", String(domRect.left));
	rect.setAttribute("y", String(domRect.top));
	rect.setAttribute("width", String(domRect.width));
	rect.setAttribute("height", String(domRect.height));
}
function setupOverlay() {
	...
	const toggle = createElement("div", "vue-inspector-toggle");
	toggle.setAttribute("role", "button");
	toggle.tabIndex = 0;
	toggle.setAttribute("aria-label", "Toggle Vue Inspector");
	toggle.setAttribute(KEY_IGNORE, "true");
	const logo = createLogoSvg();
	const banner = createElement("a", "vue-inspector-banner");
	banner.href = "https://github.com/webfansplz/vite-plugin-vue-inspector";
	banner.target = "_blank";
	banner.rel = "noreferrer";
	banner.innerHTML = "<div>vite-plugin-vue-inspector</div><div class=\"tip\">Click on a element › Open IDE › Link to File</div>";
	toggle.append(logo, banner);
	container.appendChild(toggle);
	const svg = document.createElementNS("http://www.w3.org/2000/svg", "svg");
	svg.classList.add("vue-inspector-svg");
	svg.setAttribute("width", "100%");
	svg.setAttribute("height", "100%");
	const mainRect = createRect("#42b88325", "#42b88350");
	svg.appendChild(mainRect);
	container.appendChild(svg);
	const floats = createElement("div", "vue-inspector-floats");
	const title = createElement("div", "vue-inspector-title");
	const tip = createElement("div", "tip", "Click to go to the file");
	floats.append(title, tip);
	container.appendChild(floats);
	document.body.appendChild(container);
	const cleanupHover = events.on("hover", update);
	const cleanupClick = events.on("click", handleClick);
	const cleanupEnabled = events.on("enabled", () => client.onEnabled());
	const cleanupDisabled = events.on("disabled", () => {
		closeOverlay();
		client.onDisabled();
	});
	...
	toggle.addEventListener("click", (event) => {
		if (event.target?.closest?.(".vue-inspector-banner")) return;
	...
	const stopToggleEffect = watchEffect(() => {
		const enabled = isEnabled.value;
		setLogoColors(logo, enabled ? [
			"#42D392",
			"#213547",
			"#42b883"
		] : [
		...
		toggle.style.display = toggleButtonVisibility === "always" || toggleButtonVisibility === "active" && enabled ? "block" : "none";
		const [x, y] = (inspectorOptions.toggleButtonPos || "top-right").split("-");
		applyStyles(toggle, {
			top: "",
			right: "",
			bottom: "",
	...
	const stopMainEffect = watchEffect(() => {
		const visible = state.isVisible && state.main?.rect;
		const transition = state.isAnimated ? `all ${ANIMATE_DURATION} ease-in` : "none";
		mainRect.style.transition = transition;
		floats.style.transition = transition;
		if (!visible) {
			mainRect.style.opacity = "0";
			floats.style.opacity = "0";
			return;
		}
		mainRect.style.opacity = "1";
		floats.style.opacity = "1";
		setRect(mainRect, new DOMRect(position.x, position.y, position.width, position.height));
		title.textContent = `${linkParams.title}:${linkParams.line}:${linkParams.column}`;
		const margin = 10;
		const floatsWidth = floats.clientWidth;
		const floatsHeight = floats.clientHeight;
		let left = position.x + position.width / 2;
		let top = position.y + position.height + 5;
		left = Math.max(margin, left);
		left = Math.min(left, window.innerWidth - floatsWidth - margin);
		if (left < floatsWidth / 2) left = floatsWidth / 2 + margin;
		top = Math.max(margin, top);
		top = Math.min(top, window.innerHeight - floatsHeight - margin);
		floats.style.left = `${left}px`;
		floats.style.top = `${top}px`;
	});
	const subMap = /* @__PURE__ */ new Map();
	const stopSubEffect = watchEffect(() => {
		const toRemove = new Set(subMap.keys());
		for (const { id, rect } of state.sub.rects || []) {
			toRemove.delete(id);
			let cover = subMap.get(id);
			if (!cover) {
				cover = createRect("#8482", "#8488");
				svg.appendChild(cover);
				subMap.set(id, cover);
			}
			cover.style.transition = state.isAnimated ? `all ${ANIMATE_DURATION} ease-in` : "none";
			cover.style.opacity = state.isVisible ? "1" : "0";
			setRect(cover, rect);
		}
		for (const id of toRemove) {
			subMap.get(id)?.remove();
			subMap.delete(id);
		}
	});
	window.__VUE_INSPECTOR__ = client;
	isEnabled.value = Boolean(inspectorOptions.enabled);
	const dispose = () => {
		stopToggleEffect();
		stopMainEffect();
		stopSubEffect();
		...
		container.remove();
		if (window.__VUE_INSPECTOR__ === client) delete window.__VUE_INSPECTOR__;
		if (window[DISPOSE_KEY] === dispose) delete window[DISPOSE_KEY];
	};
	window[DISPOSE_KEY] = dispose;
	return dispose;
}
const dispose = setupOverlay();
if (import.meta.hot) import.meta.hot.dispose(dispose);
//#endregion
export { ElementTraceInfo, events, findTraceAtPointer, findTraceFromElement, findTraceFromVNode, getInternalStore, hasData, isEnabled, lastMatchedElement, recordPosition, state };
